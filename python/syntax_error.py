...
def format_error(e, file):
	if isinstance(e, lark.exceptions.UnexpectedEOF):
		...
		print(f"{Fore.RED}{Style.BRIGHT}Error{Style.RESET_ALL}: Unexpected end of file, expected: [{formatted_chars}]")
		print(f"{Fore.CYAN}{spaces_arrow}--> {Fore.BLUE}{file.name}:{len(file.lines) + 1}:0")
