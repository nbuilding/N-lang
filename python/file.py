...
class File:
    def __init__(self, file=None, tab_length=4, name=None):
        ...
        if file is not None:
            self.lines = [line.rstrip().replace('\t', ' ' * tab_length)
                          for line in file]
    ...
    def display(
        ...
        if start_line == end_line:
            ...
            for line_num, line in enumerate(self.get_lines(
                    start_line, end_line), start=start_line):
                if line_num == start_line:
                    line = line[:start_col - 1] + Fore.RED + \
                        line[start_col - 1:] + Style.RESET_ALL
                elif line_num == end_line:
                    line = Fore.RED + line[:end_col - 1] + \
                        Style.RESET_ALL + line[end_col - 1:]
                else:
