...
class TypeCheckError:
    ...
    def display(self, display_type, file):
        ...
        if display_type == "error":
            ...
            raise ValueError(
                "%s is not a valid display type for TypeCheckError." %
                display_type)
        output += ": %s\n" % self.message
    ...
    def __repr__(self):
        return 'TypeCheckError(%s, %s)' % (
            repr(self.datum), repr(self.message))
...
def display_type(n_type, color=True):
    ...
    if isinstance(n_type, str):
        ...
        display = ' -> '.join(('(%s)' if isinstance(type, tuple) else '%s') %
                              display_type(type, False) for type in n_type)
        ...
        if (isinstance(n_type[0], lark.Token)):
            if (n_type[0].type == "LIST"):
                if len(n_type) >= 2:
                    ...
                    display = 'list[]'
        if display == "":
            display = '(' + ', '.join(display_type(type, False)
                                      for type in n_type) + ')'
        ...
        display = "{ %s }" % "; ".join(
            '%s: %s' % (key,
                        display_type(
                            value,
                            False)) for key,
            value in n_type.items())
        if len(n_type) == 0:
            display = "{}"
    elif isinstance(n_type, NTypeVars):
        display = n_type.name
        if len(n_type.typevars) > 0:
            display += '[%s]' % ', '.join(display_type(typevar, False)
                                          for typevar in n_type.typevars)
