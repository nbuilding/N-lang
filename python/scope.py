...
from type import NType, NGenericType, NAliasType, NTypeVars, NModule, apply_generics, apply_generics_to, \
    resolve_equal_types, NClass
...
class Scope:
    ...
    def get_module_type(self, module_type, err=True):
        *modules, type_name = module_type.children
        if len(modules) > 0:
            ...
            if n_type is None:
                self.errors.append(
                    TypeCheckError(type_name, "The module doesn't export a type `%s`." % type_name.value))
                return None
        else:
            n_type = self.get_type(type_name.value, err=err)
            if n_type is None:
                self.errors.append(TypeCheckError(module_type,
                                                  "I don't know what type you're referring to by `%s`." % type_name.value))
                return None
        if n_type == "invalid":
            return None
        else:
    ...
    def parse_type(self, tree_or_token, err=True):
        if type(tree_or_token) == lark.Tree:
            if tree_or_token.data == "with_typevars":
                ...
                if typevar_type is None:
                    ...
                    if len(typevars) < len(typevar_type.typevars):
                        self.errors.append(TypeCheckError(tree_or_token, "%s expects %d type variable(s)." % (
                        display_type(typevar_type), len(typevar_type.typevars))))
                        return None
                    elif len(typevars) > len(typevar_type.typevars):
                        self.errors.append(TypeCheckError(tree_or_token, "%s only expects %d type variable(s)." % (
                        display_type(typevar_type), len(typevar_type.typevars))))
                        return None
                    return typevar_type.with_typevars(parsed_typevars) if None not in parsed_typevars else None
                else:
                    self.errors.append(TypeCheckError(tree_or_token,
                                                      "%s doesn't take any type variables." % display_type(
                                                          typevar_type)))
                    return None
                ...
                record_type = {entry.children[0].value: self.parse_type(entry.children[1], err=err) for entry in
                ...
                if n_type is None:
                    ...
                    self.errors.append(TypeCheckError(tree_or_token, "%s expects %d type variables." % (
                ...
                if type(func_types[0]) == lark.Tree and func_types[0].data == "generic_declaration":
                    ...
                    for generic in generics.children:
                        if generic.value in scope.types:
                            self.errors.append(
                                TypeCheckError(generic, "You already defined a generic type with this name."))
                ...
                self.errors.append(TypeCheckError(tree_or_token,
                                                  "Internal problem: encountered a type annotation type %s." % tree_or_token.data))
                return None
            ...
            self.errors.append(TypeCheckError(tree_or_token,
                                              "Internal problem: encountered a type annotation token type %s." % tree_or_token.data))
            return None
        ...
        if isinstance(pattern, dict):
            ...
            if is_dict:
                ...
                if len(unused_keys) > 0:
                    self.errors.append(TypeCheckError(src,
                                                      "%s (%s) has field(s) %s, but you haven't destructured them. (Hint: use `_` to denote unused fields.)" % (
                                                      display_type(value_or_type), path_name, ", ".join(unused_keys))))
            else:
                if warn:
            ...
            for key, (sub_pattern, parse_src) in pattern.items():
                ...
                if is_dict and value is None:
                    if warn:
                        self.errors.append(TypeCheckError(parse_src,
                ...
                valid = self.assign_to_pattern((sub_pattern, parse_src), value, warn,
                                               "%s.%s" % (path or "<record>", key), public, certain=certain)
                if not valid:
                    return False
            ...
            if is_tuple and len(pattern) != len(value_or_type):
                if warn:
                    if len(pattern) > len(value_or_type):
                        ...
                        self.errors.append(TypeCheckError(parse_src, "I can't destructure %d items from a %s." % (
            ...
            for i, (sub_pattern, parse_src) in enumerate(pattern):
                ...
                valid = self.assign_to_pattern((sub_pattern, parse_src), value, warn, "%s.%d" % (path or "<tuple>", i),
                                               public, certain=certain)
                if not valid:
                    return False
            ...
            if warn:
                ...
                if not isinstance(value_or_type, EnumType):
                    ...
                    if variant_types is None:
                        self.errors.append(TypeCheckError(src, "%s has no variant %s because it's a %s." % (
                        path_name, pattern.variant, display_type(value_or_type))))
                        problem = True
                    elif len(pattern.patterns) < len(variant_types):
                        self.errors.append(TypeCheckError(src,
                                                          "Variant %s has %d fields, but you only destructure %d of them." % (
                                                          pattern.variant, len(variant_types), len(pattern.patterns))))
                        problem = True
                    elif len(pattern.patterns) > len(variant_types):
                        self.errors.append(TypeCheckError(pattern.patterns[len(variant_types)][1],
            ...
            if warn and not problem and certain and len(value_or_type.variants) > 1:
                self.errors.append(TypeCheckError(src,
                                                  ...
                                                  path_name, pattern.variant, (
                                                      value_or_type.variants[1] if value_or_type.variants[0][
                                                                                       0] == pattern.variant else
                                                      value_or_type.variants[0])[0])))
                problem = True
            for i, (sub_pattern, parse_src) in enumerate(pattern.patterns):
                ...
                valid = self.assign_to_pattern((sub_pattern, parse_src), value, warn,
                                               "%s.%s#%d" % (path or "<enum>", pattern.variant, i + 1), public,
                                               certain=certain)
                if not valid:
                    return False
            ...
            if warn and certain:
                self.errors.append(TypeCheckError(src,
                                                  ...
                                                  path_name, len(pattern),
                                                  "have two items" if len(pattern) == 0 else "be empty")))
            if not warn and len(value_or_type) != len(pattern):
                return False
            for i, (sub_pattern, parse_src) in enumerate(pattern):
                valid = self.assign_to_pattern((sub_pattern, parse_src), contained_type if warn else value_or_type[i],
                                               warn, "%s[%d]" % (path or "<enum variant>", i), public, certain=certain)
                if not valid:
                    return False
    ...
    async def eval_command(self, tree):
        ...
        if command.data == "imp":
            ...
            for constructor in constructors.children:
                ...
                public = any(modifier.type == "PUBLIC" for modifier in modifiers.children)
                if len(types) >= 1:
                    self.variables[constructor_name] = NativeFunction(self, [("idk", arg_type) for arg_type in types],
                                                                      enum_type, EnumValue.construct(constructor_name),
                                                                      public=public)
                else:
    ...
    def get_name_typevars(self, type_def):
        ...
        for typevar_name in type_typevars:
            ...
            if typevar_name.value in scope.types:
                self.errors.append(
                    TypeCheckError(typevar_name, "You've already used the generic type `%s`." % typevar_name.value))
    ...
    def type_check_expr(self, expr):
        ...
        if expr.data == "ifelse_expr":
            ...
            if condition.data == "conditional_let":
                ...
                if cond_type is not None and cond_type != "bool":
                    self.errors.append(TypeCheckError(condition,
                                                      "The condition here should be a boolean, not a %s." % display_type(
                                                          cond_type)))
            ...
            if incompatible:
                self.errors.append(TypeCheckError(expr,
            ...
            if type(condition.children[0]) is lark.Token:
                if condition.children[0].value == "true":
                    self.warnings.append(
                        TypeCheckError(condition, "The else statement of the expression will never run."))
                if condition.children[0].value == "false":
                    self.warnings.append(
                        TypeCheckError(condition, "The if statement of the expression will never run."))
            return return_type
        elif expr.data == "function_def" or expr.data == "anonymous_func":
            if expr.data == "function_def":
                arguments, returntype, codeblock = expr.children
            ...
            dummy_function = Function(self, arguments, wrap_scope.parse_type(returntype, err=False), codeblock,
                                      generic_types)
            ...
            if returnvalue is None:
                ...
                if n_cmd_type.is_type(dummy_function.returntype):
                    ...
                    if incompatible:
                        self.errors.append(TypeCheckError(codeblock,
                    ...
                    self.errors.append(TypeCheckError(codeblock,
                                                      ...
                                                      display_type(dummy_function.returntype), display_type("unit"))))
            ...
            if not isinstance(func_type, tuple):
                self.errors.append(
                    TypeCheckError(expr, "You tried to call a %s, which isn't a function." % display_type(func_type)))
                return None
            *arg_types, return_type = func_type
            ...
            for n, (argument, arg_type) in enumerate(zip(arguments, arg_types), start=1):
                ...
                if incompatible:
                    if expr.data == "function_callback":
                        self.errors.append(TypeCheckError(argument,
                                                          "%s's argument #%d should be a %s, but you gave a %s." % (
                                                          display_type(func_type), n, display_type(resolved_arg_type),
                                                          display_type(check_type))))
                    else:
                        if n == len(arguments):
                            self.errors.append(TypeCheckError(argument,
                                                              "This left operand of |>, which I pass as the last argument to %s, should be a %s, but you gave a %s." % (
                                                              display_type(func_type), display_type(resolved_arg_type),
                                                              display_type(check_type))))
                        else:
                            self.errors.append(TypeCheckError(argument,
                                                              "The argument #%d here should be a %s because the function is a %s, but you gave a %s." % (
                                                              n, display_type(resolved_arg_type),
                                                              display_type(func_type), display_type(check_type))))
            if len(arguments) > len(arg_types):
                self.errors.append(TypeCheckError(expr, "A %s has %d argument(s), but you gave %d." % (
            ...
            if value_type is None:
                ...
                self.errors.append(
                    TypeCheckError(value, "You can only get fields from records, not %s." % display_type(value_type)))
                return None
            elif field.value not in value_type:
                self.errors.append(
                    TypeCheckError(expr, "%s doesn't have a field `%s`." % (display_type(value_type), field.value)))
                return None
            else:
            ...
            if n_cmd_type.is_type(value_type):
                ...
                self.errors.append(TypeCheckError(expr,
                                                  "You can only use the await operator on cmds, not %s." % display_type(
                                                      value_type)))
            ...
            if parent_function is None:
                ...
                self.errors.append(TypeCheckError(expr,
                                                  "You can only use the await operator in a function that returns a cmd, but the surrounding function returns a %s." % display_type(
        ...
        if len(expr.children) == 2 and type(expr.children[0]) is lark.Token:
            ...
            if types:
                ...
                self.errors.append(TypeCheckError(expr, "I don't know how to use %s on a %s." % (
        ...
        if len(expr.children) == 3 and type(expr.children[1]) is lark.Token:
            ...
            if types:
                ...
                for left_operand_type, right_operand_type, result_type in types:
                    ...
                    if incompatible:
                        continue
                    return apply_generics_to(result_type, generics)
                self.errors.append(TypeCheckError(expr, "I don't know how to use %s on a %s and %s." % (
                operation.type, display_type(left_type), display_type(right_type))))
                return None
            elif expr.data == "compare_expression":
                left, comparison, right = expr.children
                if left.data == "compare_expression":
                ...
                if incompatible:
                    self.errors.append(TypeCheckError(comparison,
                ...
                if comparison.type != "EQUALS" and comparison.type != "NEQUALS" and comparison.type != "NEQUALS_QUIRKY":
                    if resolved_type is not None and resolved_type not in comparable_types:
                        self.errors.append(
                            TypeCheckError(comparison, "I don't know how to compare %s." % display_type(resolved_type)))
            ...
            for i, item_type in enumerate(rest):
                ...
                if incompatible:
                    self.errors.append(TypeCheckError(expr.children[i + 1],
            ...
            if os.path.isfile(file_path):
                ...
                if holder == {}:
                    self.warnings.append(
                        TypeCheckError(expr.children[0], "There was nothing to import from %s" % expr.children[0]))
            ...
            if None in record_type.values():
                ...
                return record_type
        self.errors.append(
            TypeCheckError(expr, "Internal problem: I don't know the command/expression type %s." % expr.data))
        return None
    ...
    def type_check_command(self, tree):
        if tree.data == "code_block":
            ...
            for instruction in tree.children:
                ...
                if exit and exit_point is None:
                    ...
                    warned = True
                    self.warnings.append(TypeCheckError(exit_point,
                                                        "There are commands after this return statement, but I will never run them."))
            parent_function = self.get_parent_function()
            return exit_point
        elif tree.data != "instruction":
            self.errors.append(
                TypeCheckError(tree, "Internal problem: I only deal with instructions, not %s." % tree.data))
            return False
        ...
        if command.data == "imp":
            ...
            if import_name in self.variables:
                self.errors.append(
                    TypeCheckError(command.children[0], "You've already used the name `%s`." % import_name))
            try:
                ...
                self.errors.append(TypeCheckError(command.children[0],
                                                  "`%s` isn't a compatible native library." % command.children[0]))
            except KeyError:
                self.errors.append(
                    TypeCheckError(command.children[0], "I can't find the native library `%s`." % command.children[0]))
            ...
            if iterable_type is not None:
                if iterated_type is None:
                    self.errors.append(
                        TypeCheckError(iterable, "I can't loop over a %s." % display_type(iterable_type)))
                elif ty == 'infer':
                    ty = iterated_type
                elif ty != iterated_type:
                    self.errors.append(TypeCheckError(ty, "Looping over a %s produces %s values, not %s." % (
                    display_type(iterable_type), display_type(iterated_type), display_type(ty))))
            scope = self.new_scope()
                ...
                if n_cmd_type.is_type(parent_function.returntype):
                    ...
                    if incompatible:
                        self.errors.append(TypeCheckError(command.children[0],
                                                          ...
                                                          display_type(parent_function.returntype.typevars[0]))))
                elif incompatible:
                    self.errors.append(TypeCheckError(command.children[0],
            ...
            if ty == 'infer':
                ...
                if incompatible:
                    self.errors.append(TypeCheckError(value,
            ...
            for constructor in constructors.children:
                ...
                if constructor_name.value in self.variables:
                    self.errors.append(TypeCheckError(constructor_name,
                                                      "You've already defined `%s` in this scope." % constructor_name.value))
                if len(types) >= 1:
                    self.variables[constructor_name.value] = NativeFunction(self,
                                                                            [("idk", arg_type) for arg_type in types],
                                                                            enum_type, id, public=public)
                else:
            ...
            if type(alias_raw_type) == lark.Tree and alias_raw_type.data == "recorddef" and isinstance(alias_type,
                                                                                                       dict):
                if alias_name in self.variables:
                    self.warnings.append(TypeCheckError(alias_def,
                                                        "Type aliases for records now declare constructor functions, but `%s` already exists. In the future, this may become an error." % alias_name))
                else:
            ...
            if name.value in self.types:
                scope.errors.append(
                    TypeCheckError(name, "You've already defined the `%s` type in this scope." % name.value))
