...
export class FuncCall extends Base implements Expression, Statement {
  ...
  typeCheck(context: TypeCheckContext): TypeCheckResult {
    ...
    const paramTypes: [NType, Expression, boolean][] = this.params.map(
      (param, i) => {
        const paramExpr = context.scope.typeCheck(param);

        const vals =
          param instanceof Spread && paramExpr.type.type === 'tuple'
            ? paramExpr.type.types
            : [param];
    ...
    for (const [param, base, spread] of paramTypes) {
      ...
      if (funcType.type === 'function') {
        const argumentTypes =
          spread && param.type === 'tuple' ? param.types : [param];
