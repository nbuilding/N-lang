...
import { AliasSpec, TypeSpec } from '../../type-checker/types/TypeSpec';

export class RecordAccess extends Base implements Expression {
  value: Expression;
  field: Identifier;
  private _type?: NRecord | NModule | TypeSpec;
  ...
  typeCheck(context: TypeCheckContext): TypeCheckResult {
    ...
    if (resolved.type === 'record' || resolved.type === 'module') {
      ...
      const types = resolved.type === 'union' ? resolved.types : [resolved];
      let isTraitableType;
      for (let t of types) {
        if (!(t instanceof TypeSpec) && t.type === 'named') {
          t = t.typeSpec;
        }
        if (t instanceof TypeSpec) {
          isTraitableType = true;
          const possibleTrait = context.scope.getTrait(
            t.name,
            this.field.value,
          );
          if (possibleTrait) {
  ...
  compile(scope: CompilationScope): CompilationResult {
    ...
    if (type instanceof TypeSpec) {
      return {
        statements,
        expression: `(${scope.context.require(
          `${type.name}.${this.field.value}`,
        )})(${expression})`,
      };
